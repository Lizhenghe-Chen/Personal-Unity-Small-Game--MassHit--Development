...
public class CameraDistMain : MonoBehaviour
{
    ...
    readonly int maxRadius = 6; //should >0
    readonly int minRadius = 0; //should <=0
    ...
    {
        if (Input.GetAxis("Mouse ScrollWheel") < 0 && currentRadius[0] < maxRadius)
        {
            //SetDistance(1);
        ...
        if (Input.GetAxis("Mouse ScrollWheel") > 0 && currentRadius[0] > minRadius)
        {
            currentRadius[0] -= offset_Value;
            currentRadius[1] -= offset_Value;
            currentRadius[2] -= offset_Value;
